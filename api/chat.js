...
function detectTickers(text = '') {
  ...
  // $TICKER form, e.g. $AAPL, $tsla
  for (const m of text.matchAll(/\$([A-Za-z]{1,5})\b/g)) {
    found.add(m[1].toUpperCase());
  }

  // "หุ้น XXXX" / "ราคา XXXX" form
  for (const m of text.matchAll(/(?:หุ้น|ราคา)\s*([A-Za-z]{1,5})\b/gi)) {
    found.add(m[1].toUpperCase());
  }

  // Bare uppercase 2-5 letter tokens, filtered against common stopwords
  for (const m of text.matchAll(/\b([A-Z]{2,5})\b/g)) {
    const t = m[1];
    if (!TICKER_STOPWORDS.has(t)) found.add(t);
  }

  return [...found].slice(0, 3); // cap to avoid spamming the proxy
}
